      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end">
          <span class="dropdown-header">
            {{ $notificationsGrouped->flatten()->count() }} Notifikasi
          </span>
          ...
          <a href="/notifications" class="dropdown-item dropdown-footer">Lihat Semua Notifikasi</a>
        </div>
      </li>
      ...
      <li class="nav-item dropdown user-menu">
        <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown">
          ...
          <span class="d-none d-md-inline">
            Halo, {{ optional(Auth::user()->admin)->name }}{{ optional(Auth::user()->supervisor)->name }}{{ optional(Auth::user()->employee)->name }} ({{ Auth::user()->getRoleNames()->first() }})
          </span>
        </a>
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">{{ $group }} Notifikasi ({{ $notifs->count() }})</h5>
      ...
      <div class="modal-body">
        <form action="{{ url('/notification/bulkDestroy') }}" method="POST" id="form-{{ Str::slug($group) }}">
          ...
          <div class="d-flex justify-content-between align-items-center mb-2 p-2 border rounded notif-item" data-group="{{ Str::slug($group) }}">
            <div class="content-clickable w-100" style="cursor:pointer;">
      ...
      <div class="modal-footer">
        <button type="submit" form="form-{{ Str::slug($group) }}" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus notifikasi yang dipilih?');">Hapus</button>
        <form action="{{ url('notification/deleteGroup') }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus semua notifikasi di grup {{ $group }}?');">
          @csrf
          <input type="hidden" name="group" value="{{ $group }}">
          <button type="submit" class="btn btn-warning">Hapus Semua</button>
        </form>
...
<script>
  function toggleSelectMode(group) {
    document.querySelectorAll(`.select-mode-${group}`).forEach(cb => cb.classList.toggle('d-none'));
  }

  document.querySelectorAll('.notif-item').forEach(item => {
    item.addEventListener('contextmenu', e => {
      e.preventDefault();
      toggleSelectMode(item.dataset.group);
    });
    ...
    item.addEventListener('touchend', function() {
      if (Date.now() - this.touchStartTime > 500) {
        toggleSelectMode(this.dataset.group);
      }
    });
  });
