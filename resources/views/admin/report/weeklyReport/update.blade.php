...
<div class="app-content-header">
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-6"><h3 class="mb-0">Edit Laporan Mingguan</h3></div>
            <div class="col-sm-6">
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="card mb-4" style="border-left: 5px solid #007bff;">
                ...
                <div class="card-body">
                    <form action="{{ url('report/weeklyReport/update/'.$weeklyReport->weekReportId) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                            ...
                            <div class="bs-stepper-content pt-4">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                          ...
                                          <input type="date" class="form-control" id="report_date" name="report_date"
                                                value="{{ old('report_date', $weeklyReport->report_date) }}">
                                        </div>
                                    </div>
                                ...
                                @foreach($stepFields as $i => $step)
                                    <div id="step-{{ $i + 2 }}" class="content">
                                        ...
                                        <div class="row">
                                          @php
                                            $detail = collect($weeklyReport->weeklyReportDetail);
                                            @endphp

                                            @foreach($step['positions'] as $pos)
                                                <div class="col-md-6 mb-3">
                                                    @php
                                                    $oldFile = $weeklyReport->weeklyReportDetail->first(function ($d) use ($step, $pos) {
                                                        return $d->component === $step['key'] && $d->position === $pos;
                                                    });
                                                    @endphp
                                                    <label class="form-label">Upload {{ $pos }}</label>
                                                    <input
                                                        type="file"
                                                        name="details[{{ $step['key'] }}][{{ $pos }}]"
                                                        class="filepond"
                                                        data-file="{{ $oldFile ? asset('storage/' . $oldFile->file_path) : '' }}"
                                ...
                                <div id="step-{{ count($stepFields) + 2 }}" class="content">
                                    <div class="mb-3">
                                        <label>Catatan</label>
                                       <textarea name="note" class="form-control" rows="4">{{ $weeklyReport->note }}</textarea>
                                    </div>
...
document.querySelectorAll('input.filepond').forEach(input => {
    ...
    if (existingFileUrl) {
        fetch(existingFileUrl)
            .then(res => res.blob())
            .then(blob => {
                pond.addFile(new File([blob], existingFileUrl.split('/').pop(), { type: blob.type }));
            });
    }
});
